...
function makeFunction(func) {
  const b = func.toString();

  let parameters = b.slice(b.indexOf("(") + 1, b.indexOf(")"));

  const c = b.slice(b.indexOf("{") + 3, -2);

  const tmpFunc = Object.defineProperty(new Function(parameters, c), "name", {
    value: func.name,
  });
  return tmpFunc;
}
